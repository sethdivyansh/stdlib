...
Evaluates the mean for an laplace distribution.
...
```c
double stdlib_base_dists_laplace_mean( const double x, const double a, const double b );
```
...
int main( void ) {
    ...
    for ( i = 0; i < 25; i++ ) {
        ...
        scale = 20.0*(double)rand() / ( (double)RAND_MAX + 1.0 );
