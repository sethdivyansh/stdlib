...
#### stdlib_base_dists_normal_mean( a, b )

Returns the mean of an normal distribution.

```c
double out = stdlib_base_dists_normal_mean( 2.0, 1.0 );
// returns ~2.0
```
...
-   **µ**: `[in] double` mean.
-   **σ**: `[in] double` standard deviation.

```c
double stdlib_base_dists_normal_mean( const double µ, const double σ );
```
...
int main( void ) {
    ...
    for ( i = 0; i < 25; i++ ) {
        ...
        sigma = 20.0*(double)rand() / ( (double)RAND_MAX + 1.0 );
